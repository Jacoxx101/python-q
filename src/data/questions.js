    ...
    questions: [
      {
        ...
        hints: ["Use input() to collect each value.", "Use an f-string like f\"{name} cast {spell}...\" to build the final message.", "Remember input() always returns a string."]
      },
      {
        id: 2,
        ...
        hints: ["Use float(input(...)) to cast the string to a number.", "Divide grams by 28.35 to get ounces.", "Use an f-string with :.2f to show 2 decimal places."]
      },
      {
        id: 3,
        ...
        hints: ["import math at the top.", "Use math.sqrt() for the square root.", "Cast the inputs to int or float before doing math."]
      },
      {
        id: 4,
        ...
        hints: ["Use .lower() on the input so capitalization doesn't matter.", "Chain if / elif / else for each house.", "Add a final else for unknown traits."]
      },
      {
        id: 5,
        ...
        hints: ["Cast age to int.", "Combine conditions with 'and' and 'or'.", "Convert yes/no answers into booleans like family == 'yes'."]
      },
      {
        id: 6,
        ...
        hints: ["name[0] gives the first character.", "name[-1] gives the last.", "Use len(name) and name.upper()."]
      },
      {
        id: 7,
        ...
        hints: ["Start with minutes = 60.", "Loop while minutes > 0, subtracting 1 each time.", "Print the final message after the loop."]
      },
      {
        id: 8,
        ...
        hints: ["Create a variable total = 0.", "Loop: for g in goals: total += g.", "Use len(goals) for goal count."]
      },
      {
        id: 9,
        ...
        hints: ["Initialize: points = {'Gryffindor': 0, ...}.", "Update with points[house] += amount.", "Make sure to cast the points input to int."]
      },
      {
        id: 10,
        ...
        hints: ["Default values go in the parameter list: spell='Lumos'.", "Call with keyword args like spell='Expelliarmus'.", "Use an f-string inside the function to print."]
      }
    ]
  },
  {
    ...
    questions: [
      {
        ...
        hints: ["Store each input in its own variable.", "Use f\"...{destination}...\" to build the announcement.", "Keep values as strings — no casting needed here."]
      },
      {
        id: 12,
        ...
        hints: ["Cast with float(input(...)).", "Multiply by 0.9 for a 10% discount.", "Use :.2f to format currency."]
      },
      {
        id: 13,
        ...
        hints: ["Cast all inputs to float.", "Divide miles by mpg.", "Use :.2f for the cost."]
      },
      {
        id: 14,
        ...
        hints: ["Use .lower() on the unit input.", "Check unit == 'kg' or 'lbs' with if/elif.", "Round or format using :.2f."]
      },
      {
        id: 15,
        ...
        hints: ["Cast temp to float.", "Use an if/elif on the unit.", "Keep the formula simple and direct."]
      },
      {
        id: 16,
        ...
        hints: ["Use name.title() to capitalize words.", "Use flight.upper().", "Use :<40 in an f-string to pad to 40 chars."]
      },
      {
        id: 17,
        ...
        exampleInput: "(no input)",
        exampleOutput: "10\n9\n8\n...\n0\nLiftoff!",
        hints: ["import time at the top.", "Use time.sleep(1) inside the loop.", "Decrement your counter each iteration."]
      },
      {
        id: 18,
        ...
        hints: ["Use enumerate(passengers, start=1).", "Or use a counter variable and += 1.", "Use an f-string for the output line."]
      },
      {
        id: 19,
        ...
        hints: ["Tuples use parentheses: ('AA101', 'NYC', 'LA').", "Unpack in the loop: for num, origin, dest in flights.", "Use an f-string to format the arrow."]
      },
      {
        id: 20,
        ...
        exampleOutput: "1000",
        hints: ["Define the function with *flights in the parameter list.", "Inside, flights is a tuple — use sum(flights).", "Return the result and print it at the call site."]
      }
    ]
  },
  {
    ...
    questions: [
      {
        ...
        hints: ["Use input() to read the password.", "Compare with == 'dragon'.", "Use .lower() to make it case-insensitive."]
      },
      {
        id: 22,
        ...
        hints: ["Convert each yes/no to a boolean like sword = (answer == 'yes').", "Combine with: if sword and (shield or potion).", "Print the right message in each branch."]
      },
      {
        id: 23,
        ...
        hints: ["Use: match choice:\\n    case 'warrior': ...", "Remember the 'case _:' default at the end.", "Lowercase the input first for consistency."]
      },
      {
        id: 24,
        ...
        hints: ["rune[:3] for first 3.", "rune[-4:] for last 4.", "rune[::2] for every other, rune[::-1] for reversed."]
      },
      {
        id: 25,
        ...
        hints: ["for floor in range(1, 11):", "Use an f-string inside the loop.", "Print the final message after the loop."]
      },
      {
        id: 26,
        ...
        exampleInput: "(no input)",
        exampleOutput: "#####\n#####\n#####\n#####\n#####",
        hints: ["Outer loop: for row in range(5).", "Inner loop: for col in range(5), print('#', end='').", "Print an empty print() after the inner loop to go to the next line."]
      },
      {
        id: 27,
        ...
        hints: ["Use inventory.append('potion').", "Use inventory.insert(0, 'map').", "Use inventory.remove('shield')."]
      },
      {
        id: 28,
        ...
        hints: ["Use set(abilities) to dedupe.", "Use len() on the set.", "Print both values."]
      },
      {
        id: 29,
        ...
        hints: ["Access with grid[row][col].", "Print both values.", "Use an f-string for formatting."]
      },
      {
        id: 30,
        ...
        hints: ["Access with hero['name'].", "Update with hero['hp'] -= 20.", "Print the whole dict at the end."]
      }
    ]
  },
  {
    ...
    questions: [
      {
        ...
        hints: ["Use int(input(...)) for the age.", "Store each input in a variable.", "Use an f-string for the final line."]
      },
      {
        id: 32,
        ...
        hints: ["import math.", "Cast inputs to float.", "Use math.ceil(fuel) on the final number."]
      },
      {
        id: 33,
        ...
        hints: ["Syntax: 'OK' if level >= 20 else 'WARNING'.", "Cast the input to int first.", "Print status inside an f-string."]
      },
      {
        id: 34,
        ...
        hints: ["Use f\"{speed:.1f}\" for 1 decimal place.", "Use f\"{fuel:.0%}\" for a percentage.", "Combine both in one f-string."]
      },
      {
        id: 35,
        ...
        hints: ["Cast principal and rate to float, years to int.", "Use a counter variable and loop while it < years.", "Format the final balance with :.2f."]
      },
      {
        id: 36,
        ...
        hints: ["Use: for name, dist in distances.items().", "Use an f-string for the line.", "Make sure the keys and values are in the expected format."]
      },
      {
        id: 37,
        ...
        hints: ["Use a while True loop that breaks on 'quit'.", "Look up the price with items[choice].", "Handle invalid items with an if choice in items check."]
      },
      {
        id: 38,
        ...
        hints: ["Syntax: [a for a in ages if a >= 18].", "Assign to a new variable, then print.", "The condition goes after the source list."]
      },
      {
        id: 39,
        ...
        hints: ["Define with def build_profile(**info):", "Loop over info.items() inside the function.", "Call the function with keyword arguments only."]
      },
      {
        id: 40,
        ...
        hints: ["import random at the top.", "Use random.choice(names) and random.choice(planets).", "Print with an f-string."]
      }
    ]
  },
  {
    ...
    questions: [
      {
        ...
        hints: ["Each input gets its own variable.", "Use a single f-string for the final line.", "No casting needed — everything stays a string."]
      },
      {
        id: 42,
        ...
        hints: ["Use .strip(), .lower(), .replace('!', '').", "For multiple spaces, use .replace('  ', ' ') a few times, or split/join.", "Chain the calls for readability."]
      },
      {
        id: 43,
        ...
        hints: ["Use a list of tuples: [('What weapon?', 'knife'), ...].", "Loop and compare the user's input (lowercased) to the correct answer.", "Increment score only on a match."]
      },
      {
        id: 44,
        ...
        hints: ["Use int(input(...)) for each guess.", "while guess != secret: ...", "Increment a guess counter each iteration."]
      },
      {
        id: 45,
        ...
        hints: ["Use random.choice on a list.", "Cover all the winning cases with if/elif.", "Handle ties with a separate branch."]
      },
      {
        id: 46,
        ...
        hints: ["import random.", "Use range(n) for the for loop.", "Keep a running total variable."]
      },
      {
        id: 47,
        ...
        hints: ["[s for s in suspects if s.startswith('Mr. ')].", "Note the space after 'Mr.' to exclude 'Mrs.'.", "Print the resulting list."]
      },
      {
        id: 48,
        ...
        hints: ["match role:\\n    case 'butler': ...", "Remember case _: for the default.", "Lowercase the role first."]
      },
      {
        id: 49,
        ...
        hints: ["Keep balance as a variable in main and pass it to functions.", "Each function returns the new balance; reassign balance = deposit(balance, amt).", "Break out of the loop when the user picks quit."]
      },
      {
        id: 50,
        ...
        hints: ["Use ord(ch) - ord('a') to get 0-25, then mod 26 after shifting.", "Use chr(...) to convert back to a letter.", "Only shift alphabetic characters — leave spaces and punctuation alone."]
      }
    ]
  }
];
